...
class CyberSourceSopNotificationTest < Test::Unit::TestCase
  ...
  def test_accessors
    ...
    assert_equal "REJECT", @cyber_source_sop.status
    assert_equal nil, @cyber_source_sop.transaction_id
    assert_equal "1000", @cyber_source_sop.item_id
    assert_equal "50.0", @cyber_source_sop.gross
    assert_equal "aed", @cyber_source_sop.currency
    assert @cyber_source_sop.test?
  end

  def test_compositions
    assert_equal Money.new(5000, 'aed'), @cyber_source_sop.amount
  end

  def test_reason_code
    assert_equal '102', @cyber_source_sop.reason_code
  end

  def test_reason
    assert_equal 'One or more fields contains invalid data', @cyber_source_sop.reason
  end

  def test_missing_fields
    assert_equal 4, @cyber_source_sop.missing_fields.size
  end

  def test_invalid_fields
    assert_equal 1, @cyber_source_sop.invalid_fields.size
  end
